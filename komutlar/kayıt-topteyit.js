...
if(!message.member.roles.cache.some(r => ['KULLANICAK ROL ID 1', 'KULLANICAK ROL ID 2'].includes(r.id)) && (!message.member.hasPermission("ADMINISTRATOR")))return message.reply("Bu Komutu Kullanmak İçin Yetkiniz Bulunmamakta.")  

  let uye = message.mentions.users.first() || message.author;
let bilgi = db.get(`yetkili.${uye.id}.toplam`);
let başlık = "Top Kayıt Listesi"
